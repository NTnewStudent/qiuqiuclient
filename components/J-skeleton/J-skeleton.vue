<template>
  <view >
    <view v-if="loading" class="skeleton bg-white" :class="{ animate: animate }"  :style="{ justifyContent: flexType}">
			<!-- 轮播图 -->
			<view
			  v-if="imgTitle"
			  class="skeleton-imgTitle bg-white"
			  style="width: 95%;border-radius: 10px;height: 100px;display: block;"
			></view>
			<!-- 头像图 -->
      <view
        v-if="showAvatar && !imgTitle"
        class="skeleton-avatar bg-white"
      ...
      ></view>
			<!-- 文字条 -->
      <view class="skeleton-content bg-white " v-if="showTitle  && !imgTitle">
        <view  class="skeleton-title bg-white" :style="{ width: titleWidth }"></view>
        <view class="skeleton-rows bg-white">
          <view
            class="skeleton-row-item bg-white"
